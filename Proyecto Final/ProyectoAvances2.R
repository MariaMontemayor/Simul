...
for (vacuna in pv) {
  ...
  base_epidemia <- cbind(epidemia, vacuna)
  base <-  rbind(base, base_epidemia)
}

png("diagramaviolin.png", width=600, height=300)
plot(1:length(epidemia), 100 * epidemia / n, xlab="Tiempo", ylab="Porcentaje de infectados")
graphics.off()

base[,1] <- base[,1]/50 * 100
base <- as.data.frame(base)
colnames(base) <- c("Porcentaje_Infectados","Probabilidad_Vacuna")
base$Probabilidad_Vacuna <- as.factor(base$Probabilidad_Vacuna)

g2 <- ggplot(base, aes(x=Probabilidad_Vacuna, y=Porcentaje_Infectados, fill=Probabilidad_Vacuna)) + 
  geom_violin() 
g2 + geom_jitter(shape=18, position=position_jitter(0.2))
